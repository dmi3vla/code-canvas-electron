...
import { GitFork, LayoutDashboard, Info, FileJson, RefreshCw, Settings, FolderOpen } from 'lucide-react';
...
import { useExtensionCommands, useVsCodeApi, useElectronApi, isElectronStandalone } from '../extensionBridge';
import type {
  Codemap,
  CodemapSuggestion,
  CodemapHistoryItem,
...
export const App: React.FC = () => {
  ...
  // Settings panel state (Electron only)
  const [showSettings, setShowSettings] = useState(false);
  const [projectName, setProjectName] = useState<string | null>(null);

  // Load project name on mount (Electron only)
  useEffect(() => {
    if (electronApi) {
      electronApi.project.getCurrent().then((p) => {
        if (p) setProjectName(p.name);
      });
    }
  }, [electronApi]);

  // Listen for extension messages in Electron mode
  useEffect(() => {
    if (!electronApi) return;
    const unsubscribe = electronApi.onExtensionMessage((message: ExtensionToWebviewMessage) => {
      switch (message.type) {
        case 'update':
          setState((prev) => ({
            ...prev,
            ...
            page: (!message.codemap && !message.isProcessing && prev.page === 'detail') ? 'home' : prev.page,
          }));
          break;
        case 'setQuery':
          setState((prev) => ({ ...prev, query: message.query }));
          break;
        case 'navigate':
          setState((prev) => ({ ...prev, page: message.page }));
          break;
      }
    });
    commands.ready();
  ...
  if (state.page === 'home') {
    return (
      <div className="app-container">
        {/* Settings & Project bar (Electron only) */}
        {electronApi && (
          <div className="electron-toolbar">
            <button
              className="electron-toolbar-btn"
              onClick={async () => {
                const result = await commands.openProject();
                if (result) setProjectName(result.name);
              }}
            ...
            <button
              className="electron-toolbar-btn settings-trigger"
